...
/// Main MapSection view responsible for displaying the map,
/// managing user location, dropped pins, and showing alerts.
    ...
    var body: some View {
        VStack {
            if let userInitialPosition = locationViewModel.userLocation {
                mapContent(
                    userInitialPosition: userInitialPosition,
        ...
        .alert("Location", isPresented: $showInfo, presenting: placeFormatted) {
            _ in
        ...
        .alert("Error", isPresented: $showFailed, presenting: error) { _ in
            ...
            Text(error.errorDescription ?? "Unknown error").foregroundColor(
                .red
            )
        }
    }
...
extension MapSection {
    ...
    /// View builder for the main map content including the TapMapView,
    /// handling initial pin drop and taps on the map.
    @ViewBuilder
    fileprivate func mapContent(
        userInitialPosition: CLLocationCoordinate2D,
    ...
    fileprivate func dropInitialPinIfNeeded(at location: CLLocationCoordinate2D)
    {
    ...
    fileprivate func handleMapTap(at coordinate: CLLocationCoordinate2D) {
